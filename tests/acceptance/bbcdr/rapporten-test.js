...
module('Acceptance | bbcdr/rapporten', function (hooks) {
  ...
  test('visiting /bbcdr/rapporten shows a button to create a new report', async function (assert) {
    ...
    assert.equal(
      currentRouteName(),
      'bbcdr.rapporten.new',
      'Clicking the create button opens the create panel'
    );

    await click('[data-test-loket=bbcdr-create-report-btn]');
    assert.equal(
      currentRouteName(),
      'bbcdr.rapporten.index',
      'Clicking the create button again closes the create panel'
    );
  });
  ...
  test('clicking the detail link on bbcdr report opens the bbcdr report details', async function (assert) {
    ...
    assert.equal(
      currentRouteName(),
      'bbcdr.rapporten.edit',
      'Clicking the details link opens the detail panel'
    );
    assert.ok(
      currentURL().endsWith('/1234'),
      'Clicking the details link opens the details of the selected report'
    );

    await click(
      '[data-test-loket=bbcdr-report-edit-header] a[data-test-loket=bbcdr-close-panel-btn]'
    );
    assert.equal(
      currentRouteName(),
      'bbcdr.rapporten.index',
      'Clicking the close icon in the header closes the detail panel'
    );
    ...
    await click(
      '[data-test-loket=bbcdr-report-edit-buttons] a[data-test-loket=bbcdr-close-panel-btn]'
    );
    assert.equal(
      currentRouteName(),
      'bbcdr.rapporten.index',
      'Clicking the close button in the footer closes the detail panel'
    );
  });
});
