...
module('Acceptance | toezicht index', function (hooks) {
  ...
  test('authenticated users can visit /toezicht/inzendingen', async function (assert) {
    ...
    assert.equal(
      currentURL(),
      '/toezicht/inzendingen',
      'user is on super-secret-url'
    );
  });
  ...
  test('Making a new melding works', async function (assert) {
    ...
    assert.equal(
      currentURL(),
      '/toezicht/inzendingen/new',
      'Clicked the "make new" button and our user is on the "new toezicht" route'
    );
    assert
      .dom('[data-test-loket=next-step-button]')
      .hasAttribute(
        'class',
        /\bdisabled\b/,
        'The "proceed" button is disabled by default'
      );
    ...
    assert.equal(
      currentURL(),
      '/toezicht/inzendingen',
      'Clicked again and the user is now back on our secret url'
    );
  });
  ...
  test('The inzending index page has a correct title', async function (assert) {
    ...
    assert
      .dom(`[data-test-loket=inzendingen-page-title]`)
      .includesText(sectionTitle, 'Title is correct');
    assert
      .dom(`[data-test-loket=inzendingen-page-title]`)
      .includesText(CLASSIFICATION_LABEL, 'Classification is correct');
  });
});
