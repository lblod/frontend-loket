...
module('Integration | Component | berichtencentrum/conversatie-view-header', function(hooks) {
  ...
  test('the compoenent renders and shows a dossiernummer, a typeCommunicatie, a cross and a betreft', async function(assert) {
    await render(hbs`{{berichtencentrum/conversatie-view-header conversatie=model}}`);

    assert.dom(`[data-test-loket=berichtencentrum-header-dossiernummer]`).exists();
    assert.dom(`[data-test-loket=berichtencentrum-header-type-communicatie]`).exists();
  ...
  test('it calls the given onclick handler when the cross is clicked', async function(assert) {
    ...
    await render(hbs`{{berichtencentrum/conversatie-view-header conversatie=model close=close crossClicked=crossClicked}}`);

    assert.equal(this.crossClicked, false);
    await click('[data-test-loket=berichtencentrum-header-cross]');
    assert.equal(this.crossClicked, true);
